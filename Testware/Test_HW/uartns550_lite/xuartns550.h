 ...
 */
typedef struct {
	unsigned int BaseAddress;	/**< Base address of device */
	unsigned int InputClockKHz;	/**< Input clock frequency */
	unsigned int DefaultBaudRate;	/**< Baud Rate in bps, ie 1200 */
} XUartNs550_Config;
 ...
 */
typedef struct {
	unsigned char BaudRate;	/**< In bps, ie 1200 */
 ...
 */
typedef struct {
	unsigned char *BaseAddress;	/**< Base address of device  */
	unsigned long InputClockHz;	/**< Input clock frequency in KHz */
	int IsReady;		/**< Device is initialized and ready */
	unsigned long BaudRate;		/**< Current baud rate of hw */
	unsigned char  LastErrors;		/**< The accumulated errors */
...
unsigned int XUartNs550_Recv(unsigned char *BufferPtr,
				unsigned int NumBytes);
